...
## Isabelle Sanity Target

The checked sanity proof may eventually provide:
...
These facts are sanity checks, not bounty-level artifacts by themselves. The
bounty-level direction is a general match-set/order/value theorem explaining
why possessive commitment is a subset or strict separation of greedy behavior
under an explicit context condition.

The proof must be narrow. Whole-engine `eval`/`normalization` can trip over code
generation because `pstate` contains the function-valued `capenv`; broad `simp`
over the full matcher is also not the right shape. A good next step is a small
computation relation for the fragment generated by literals, alternation,
sequencing, anchors, and quantifiers.
