...
theorem pmatch_iff_ordered_value_run:
  assumes "pordered_supported r"
...
proof
  ...
  show "\<exists>v. pval_ordered_run fuel r st v out"
    by (rule pmatch_ordered_value_complete[OF assms out])
next
...
theorem pmatch_iff_ordered_value_explains:
  assumes "pordered_supported r"
  shows "out \<in> set (pmatch fuel r st) \<longleftrightarrow>
...
proof
  assume out: "out \<in> set (pmatch fuel r st)"
  from pmatch_ordered_value_complete[OF assms out]
...
theorem pcre_fullmatch_iff_ordered_value_explains_atomic:
  assumes "pordered_supported_atomic r"
  shows "pcre_fullmatch fuel r s \<longleftrightarrow>
    (\<exists>v out. pval_ordered_run fuel r (PState [] s empty_caps) v out \<and>
      pval_explains_state (PState [] s empty_caps) v out \<and>
      pright out = [])"
proof
  assume full: "pcre_fullmatch fuel r s"
  then obtain out where both:
    "out \<in> set (pmatch fuel r (PState [] s empty_caps)) \<and>
     pright out = []"
    by (auto simp add: pcre_fullmatch_def)
  then have out_mem:
    "out \<in> set (pmatch fuel r (PState [] s empty_caps))"
    by simp
  from both have right_empty: "pright out = []"
    by simp
  from pmatch_ordered_value_complete_atomic[OF assms out_mem]
  obtain v where run:
    "pval_ordered_run fuel r (PState [] s empty_caps) v out"
    ..
  have explains: "pval_explains_state (PState [] s empty_caps) v out"
    using pval_ordered_run_explains_state[OF run] .
  show "\<exists>v out. pval_ordered_run fuel r (PState [] s empty_caps) v out \<and>
    pval_explains_state (PState [] s empty_caps) v out \<and>
    pright out = []"
  proof (intro exI[of _ v] exI[of _ out] conjI)
    show "pval_ordered_run fuel r (PState [] s empty_caps) v out"
      using run .
    show "pval_explains_state (PState [] s empty_caps) v out"
      using explains .
    show "pright out = []"
      using right_empty .
  qed
next
  assume "\<exists>v out. pval_ordered_run fuel r (PState [] s empty_caps) v out \<and>
    pval_explains_state (PState [] s empty_caps) v out \<and>
    pright out = []"
  then obtain v out where run:
    "pval_ordered_run fuel r (PState [] s empty_caps) v out"
    and right_empty: "pright out = []"
    by auto
  have out: "out \<in> set (pmatch fuel r (PState [] s empty_caps))"
    using pval_ordered_run_sound_pmatch[OF run] .
  show "pcre_fullmatch fuel r s"
    using out right_empty by (auto simp add: pcre_fullmatch_def)
qed

theorem pcre_fullmatch_language_iff_ordered_value_explains_atomic:
  assumes "pordered_supported_atomic r"
  shows "s \<in> pcre_fullmatch_language fuel r \<longleftrightarrow>
    (\<exists>v out. pval_ordered_run fuel r (PState [] s empty_caps) v out \<and>
      pval_explains_state (PState [] s empty_caps) v out \<and>
      pright out = [])"
  using pcre_fullmatch_iff_ordered_value_explains_atomic[OF assms, of fuel s]
